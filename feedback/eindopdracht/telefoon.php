<?php
    ...
    if (isset($_POST['telefoon'])    &&
        isset($_POST['lidnr'])) {
            ...
            $telefoon = sanitizeString($_POST['telefoon']);
    }
    // prepare stmt
    $stmt = $conn->prepare("INSERT INTO telefoon (telefoonnummer, lidnummer) VALUES(?,?)");
    // bind params
    $stmt->bind_param('ss', $telefoon, $lidnr);
    // execute stmt
    $stmt->execute();
    // close conn and stmt
    $stmt->close();
    ...
    <form action="email.php" method="post" style="padding: 30px; background-color: cornflowerblue;"><pre>
        Telefoon        <input type="text" name="email" required>
...
echo <<<_END
                    ...
                    <input type="submit" value="Telefoon toevoegen aan lid">
  </form>
  </pre>
  </body>
  </html>
